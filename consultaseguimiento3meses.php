...
<?php 
    ...
    $sqlQueryComentarios  = $conexion2->query("SELECT dato_personal.id, dato_personal.nombrecompleto, dato_personal.edad, dato_personal.curp, dato_personal.sexo, dato_personal.fechanacimiento, tratamiento.identificador FROM dato_personal left outer join tratamiento on tratamiento.id_paciente = dato_personal.id where tratamiento.seguimiento = '3 meses'");
    $total_registro       = mysqli_num_rows($sqlQueryComentarios);
    
	$sql = "SELECT COUNT(*) total FROM tratamiento where seguimiento = '3 meses'";
    $result = mysqli_query($conexion2, $sql);
    $fila = mysqli_fetch_assoc($result);

    $query= $conexion->prepare("SELECT dato_personal.id, dato_personal.nombrecompleto, dato_personal.edad, dato_personal.curp, dato_personal.sexo, dato_personal.fechanacimiento, tratamiento.identificador, tratamiento.seguimiento FROM dato_personal left outer join tratamiento on tratamiento.id_paciente = dato_personal.id where tratamiento.seguimiento = '3 meses'  order by dato_personal.id DESC LIMIT 9");
    if(isset($_POST['pacientes']))
{
	...
	$query=$conexion->prepare("SELECT dato_personal.id, dato_personal.nombrecompleto, dato_personal.edad, dato_personal.curp, dato_personal.sexo, dato_personal.fechanacimiento, tratamiento.identificador, tratamiento.seguimiento FROM dato_personal left outer join tratamiento on tratamiento.id_paciente = dato_personal.id  where
		...
		dato_personal.curp LIKE '%".$q."%' AND tratamiento.seguimiento = '3 meses' group by dato_personal.id");
}
        ?>
...
<table class="table table-responsive  table-bordered table-striped table-hover display" width="100%"
    ...
    <tbody>
        ...
        <div class="row border_special item-comentario" id="<?php echo $dataRegistro['id']; ?>">
            ...
            if($dataRegistro['seguimiento'] == '3 meses'){
            ?>
            <tr>
